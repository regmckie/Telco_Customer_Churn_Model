...
It makes sure that serving-time feature transformations exactly match training-time transformations.
This is CRITICAL for model accuracy and reproduction.

KEY RESPONSIBILITIES:
1. Load MLfow-logged model and feature metadata from training
...
# In prod: uses model copied to container at build time
MODEL_DIR="/app/model"
...
# Ensures the model receives features in the expected order
try:
    ...
    raise Exception(f"❌ Failed to load feature columns from training: {e}")

# --- FEATURE TRANSFORMATION CONSTANTS ---
# These mappings must exactly match those used in training
# Any changes here will cause train/serve skew and degrade the model's performance
...
def _serve_transform(df: pd.DataFrame) -> pd.DataFrame:
    """
    Applies identical feature transformations as used during model training.

    This function is important for ML production because it ensures that features
    are transformed exactly as they were during training to prevent train/serve skew.

    TRANSFORMATION PIPELINE:
    1. Clean column names and handle data types
    2. Apply deterministic binary encoding (use BINARY_MAP)
    3. One-hot encode remaining categorical features
    4. Convert boolean columns to ints
    5. Align features with training schema and order

    NOTE:
    Any changes made to this function should be reflected in training feature engineering to maintain consistency

    :param df: Telco customer dataset
    :return: A DataFrame w/ features transformed and ordered for model input
    """
    df = df.copy()

    # --- STEP 1.1: CLEAN COLUMN NAMES (REMOVE WHITESPACES ---
    df.columns = df.columns.str.strip()

    # --- STEP 1.2: NUMERIC TYPE COERCION ---
    # Make sure numeric columns are properly typed (handle string inputs)
    ...
    # Must use the exact same mappings as training
    for col, mapping in BINARY_MAP.items():
        if col in df.columns:
            df[col] = (
                ...
                .astype(int)  # Final int conversion
            )
    ...
    # --- STEP 4: BOOLEAN TO INT CONVERSION ---
    # Convert any boolean columns to int (XGBoost compatibility)
...
def predict(input_dict: dict) -> str:
    ...
    This function provides the complete inference pipeline from raw customer data to business-friendly
    prediction putput. It's called by both the FastAPI endpoint and the Gradio interface to ensure
    consistent predictions.

    PIPELINE:
    1. Convert intput dict to DF
    ...
    :param input_dict: Dict containing raw customer data with keys matching the CustomerData schema
    (18 features in total)
    :return: A human-readable prediction string
                - "Likely to churn" for high-risk customers (model prediction = 1)
                - "Not likely to churn" for low-risk customers (model prediction = 0)
    """

    # --- STEP 1:  CONVERT DICT TO DF ---
    df = pd.DataFrame([input_dict])

    # --- STEP 2: APPLY FEATURE TRANSFORMATIONS ---
    # Use same transformation pipeline as training
    df_enc = _serve_transform(df)

    # --- STEP 3: GENERATE MODEL PREDICTION ---
    # Call the laoded MLflow model for inference
    # The model returns preds in various formats depending on the ML library
    try:
    ...
    # Convert binary prediction (0/1) to human-understandable language
    if result == 1:
        return "Likely to churn"  # High-risk customer: needs intervention
    else:
        return "Not likely to churn"  # Low-risk customer: maintain normal service